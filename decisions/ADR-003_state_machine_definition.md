...
This ADR defines all states, entry/exit conditions, sub-states, backplane signals, and kill-switch logic. It incorporates decisions from ADR-001 (fault detection) and ADR-002 (configuration).
...
The main board is a PLC gateway. Its responsibilities are strictly limited to three functions:
...
- Missing pre-arm readiness (e.g., unsynchronized DC-DC phase) is **Not Ready**, not a physical fault, while `EN=0`
...
| `SYNC` | Main FPGA | Point-to-point LVDS | Edge + state-qualified | Point-to-point LVDS per slot. Generated with fixed 180° phase offset relative to `CLOCK` (`SYNC` rising aligned to `CLOCK` falling) to avoid uncertain cycle capture. `IDLE` + rising edge (`EN=0`) → reset local DC-DC divider phase (pre-arm sync). `RUN.wait` + rising edge → RUN.run (start acquisition). `RUN.run` + falling edge → RUN.stop (graceful stop). Main must enforce minimum SYNC HIGH/LOW dwell `T_sync_min` (R6) for IDLE pre-arm and RUN pulses to guarantee capture in all function-board management domains. SYNC edges during `START.*` and `ERROR.*` are ignored by all boards. |
...
2. Prefixes are reserved only for board-role-specific signals that are not shared semantics (for example `main_dcdc_sync_ready`).
...
| `T_settle` | ICD-defined (expected order: milliseconds; recommended initial value `1 ms`) | Minimum settle time after IDLE `SYNC` phase-reset before arm is valid; board characterization may justify lower/higher values |
...
2. Function-board EN-rise readiness gate violation (`dcdc_sync_ready == 0` or `sequencer_hash_valid_current_arm == 0` at accepted EN rising) must set a dedicated interlock-violation bit in `fault_vector`.
...
- `SYNC` rising edge in IDLE (`EN=0`) resets the local divider chain (÷50 baseline, ÷N DC-DC, ÷M watchdog); main enforces SYNC pulse-width constraints (`T_sync_min`), then boards wait `T_settle` before arm is permitted (ADR-004 R4)
...
- Operator sends `arm` via Ethernet to the main board only. Main checks local readiness (`main_dcdc_sync_ready`) and, if ready, asserts `EN = 1` → RUN.init
...
3. On `SYNC` rising edge, each board resets the local divider chain (÷50 baseline, ÷N DC-DC, ÷M watchdog) via a one-shot reset strobe in its local clock domain
4. Boards start (or restart) a local `T_settle` timer immediately after reset and force `dcdc_sync_ready = 0`
5. On local `T_settle` expiry, boards set `dcdc_sync_ready = 1`; the host should arm only after required boards report ready, and each function board enforces `dcdc_sync_ready == 1` on `EN` rising (otherwise trips interlock)
...
1. `dcdc_sync_ready == 0` in IDLE is a **Not Ready** condition, not a fault condition
2. While `EN=0`, a board in Not Ready does not pull `OK` low solely due to missing DC-DC phase sync
...
1. On Ethernet `arm`, main board checks its own `main_dcdc_sync_ready`
...
1. Each function board independently tracks `dcdc_sync_ready` and `sequencer_hash_valid_current_arm` in IDLE. On function boards that do not implement a sequencer (e.g., static bias/voltage boards), `sequencer_hash_valid_current_arm` must be permanently tied to `1` in hardware to passively pass the EN-rise gate.
...
|---|---|---|
| RUN.init | EN rising accepted by local EN-rise qualifier (function boards use `N_en_rise_debounce`; main enters on accepted arm command) | Arm-entry initialization: flush ADCs, apply bias voltages, pre-load sequencers, reset counters. (DC-DC phase reset is performed earlier in IDLE; see ADR-004 R4.) |
| RUN.wait | RUN.init complete | Hardware ready, monitoring SYNC continuously |
| RUN.run | SYNC rising edge | Sequencers fire, data acquisition begins with all DC-DC switching transients harmonically aligned to a predictable noise profile (alignment established during IDLE pre-arm SYNC; ADR-004 R4/R8). |
...
`EN`-rise readiness checks are normative in R8 ("EN-rise safety gate on function boards").
...
| ERROR.clear | Recovery attempt. Behavior defined by R6 rules 1–11 and R8 transition table. |

**ERROR.clear sequence:**

The `ERROR.clear` recovery behavior for both main and function boards is fully defined by the fault-path and clear semantics (rules 1–11 above) and the R8 transition table. The step-by-step procedural implementation of the `ERROR.clear` entry actions, exit boundary evaluator, and board-role-specific responsibilities is documented in the Firmware Reference Appendix.
...
| IDLE | `SYNC` rising edge and `EN == 0` | IDLE | Pre-arm sync action: reset local divider chain (÷50 baseline, ÷N DC-DC, ÷M watchdog), restart `T_settle`; main enforces `T_sync_min` HIGH/LOW dwell for deterministic capture |
| IDLE (main) | `arm` command and `main_dcdc_sync_ready == 1` | RUN.init | Main asserts `EN = 1`; function-board readiness/attestation remains host-polled and EN-rise enforced locally |
| IDLE (main) | `arm` command and `main_dcdc_sync_ready == 0` | IDLE | Main replies `ARM_REJECTED_NOT_READY_MAIN` |
| IDLE | debounced `EN` rising (`N_en_rise_debounce` samples) AND `dcdc_sync_ready == 1` AND `sequencer_hash_valid_current_arm == 1` (function boards) | RUN.init | Readiness gate passed; FPGA `relay_drive` ARM asserts after RUN.init completes and external relay stage drives the coil |
| IDLE | debounced `EN` rising (`N_en_rise_debounce` samples) AND (`dcdc_sync_ready == 0` OR `sequencer_hash_valid_current_arm == 0`) (function boards) | ERROR.init | EN-rise safety gate violation; board sets dedicated interlock-violation source bit in `fault_vector`, which sets `local_trip_summary`, asserts `OK` low, and trips globally |
...
| ERROR.run | `OK == 0` | ERROR.run | System already latched safe; FSM ignores OK bus drops (no re-entry to ERROR.init). However, late-arriving local faults must still update fault_vector and local_trip_summary per R6 rule 5. |
...
| ERROR.clear (function) | function-board exit evaluation boundary reached (R6 rule 5) with `fault_vector == 0` | START.wait | Success path: assert `clear_summary_strobe` and `clear_wd_latch` on this boundary so `local_trip_summary`, `latched_supervision_fault`, and `WD_latch` clear, and `OK` releases at START.wait entry |
| ERROR.clear (function) | function-board exit evaluation boundary reached (R6 rule 5) with `fault_vector > 0` | ERROR.init | Fault persists/new fault occurred; keep `local_trip_summary = 1` and retain fault-source bits for diagnostics |
...
1. On `EN` rising edge, the outcome is fully determined by two conditions evaluated simultaneously: `dcdc_sync_ready` and `sequencer_hash_valid_current_arm`
2. If both are `1` → transition to `RUN.init` (`dcdc_sync_ready` is set only after local `T_settle` has elapsed since the last IDLE `SYNC` rising edge)
3. If either is `0` → board must immediately set a dedicated interlock-violation source bit in `fault_vector`; by rule this sets `local_trip_summary = 1`, asserts `OK` low, and transitions to `ERROR.init`. This intentionally catches edge cases such as an individual board reboot that missed the IDLE SYNC phase-reset
...
- **Power supply:** Latch/flip-flop IC and all RESET-path logic must be powered from the always-on watchdog supply (12V LDO), not the FPGA digital rail — so RESET correctly tracks backplane signals even if local logic rails collapse.
...
*Resolved. Core states (START, IDLE, RUN, ERROR), full transition specification (guards, priorities, and timing constraints), injected-fault maintenance command model, kill-switch logic, and external relay latch/flip-flop stage requirement for function boards (R9) are settled. While armed, keep_alive lease timeout on any board trips globally via `OK` pull-down.*
