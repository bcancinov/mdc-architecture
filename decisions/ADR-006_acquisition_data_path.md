...
Video function boards produce the science data of the system: digitized detector readout. The data path deserves an explicit decision because it interacts with two existing architectural rules:

1. **Main board scope (ADR-003 R1):** the main board is a coordinator, not a data concentrator.
2. **Armed host supervision (ADR-001 R10, ADR-003 R8):** while armed, every board trips the interlock if its host-supervision timer expires. The acquisition protocol may itself provide the qualifying host interactions, so this ADR must not prematurely mandate a separate heartbeat mechanism.
...
- Instruments whose measured throughput exceeds the control-link capacity may introduce a dedicated data link in their ICD/design package; doing so does not alter the host-supervision or overrun-policy rules.
